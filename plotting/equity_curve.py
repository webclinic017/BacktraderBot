...
class EquityCurvePlotter(object):
    ...
    def get_equity_curve_data_points(self, equity_curve_df, strategy_id_data_str, exchange_str, symbol_str, timeframe_str, parameters_str):
        equity_curve_data_points = equity_curve_df.loc[(strategy_id_data_str, exchange_str, symbol_str, timeframe_str, parameters_str), "Equity Curve Data Points"]
    ...
    def generate_equity_curve_images(self, results_arr, equity_curve_df, args):
        image_counter = 0
        base_dir = self.whereAmI()
        output_path = self.get_output_equity_curve_images_path(base_dir, args)
        os.makedirs(output_path, exist_ok=True)
        print("Rendering {} equity curve images into {}".format(len(results_arr), output_path))
        for row in results_arr:
            ...
            timeframe_str = row[3]
            parameters_str = row[4]
            equity_curve_data_points_str = self.get_equity_curve_data_points(equity_curve_df, strategy_id_data_str, exchange_str, symbol_str, timeframe_str, parameters_str)
