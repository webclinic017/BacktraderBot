...
class GenericStrategy(bt.Strategy):
    ...
    def signal_open_position(self, is_long):
        ...
        if is_long:
            self.strategyprocessor.open_long_position()
            self.curr_position = 1
        else:
            self.strategyprocessor.open_short_position()
            self.curr_position = -1
        self.position_avg_price = self.data.close[0]
        self.strategyprocessor.on_open_position_trade_managers(self.position_avg_price, is_long)
    ...
    def next(self):
        ...
        if self.strategyprocessor.on_next_trade_managers():
            self.curr_position = 0
            self.position_avg_price = 0
            self.strategyprocessor.notify_analyzers()
