...
class ShotsPnlCalculator(object):
    ...
    def write_pnl_data_to_file(self, args, df, shot_type):
        ...
        # Save it
        filename = '{}/shots-pnl-{}-{}-{}-{}-mt.csv'.format(output_path, args.exchange, symbol_type_str, args.symbol, shot_type)
        df.to_csv(filename)
    ...
    def write_best_pnl_rows_to_file(self, args, total_shots_count, df, shot_type):
        ...
        filename = '{}/shots-best-pnl-{}-{}-mt.csv'.format(output_path, args.exchange, symbol_type_str)
    ...
    def calculate_shot_trials(self, shot, c_distance, c_buffer, c_tp, c_sl):
        shot_trials_pnl_arr = []
        ...
        for trd in trials_range:
            ...
            if shot_trial_end < -c_sl or shot_bounce_end < -c_sl:
                ...
                if shot_bounce_end >= c_tp:
                    ...
                    assumed_tp = DEFAULT_MT_MIN_TP_PCT
        ...
        for c_idx, c_dict in enumerate(combinations):
            ...
            for index, shot_group in groups_df.iterrows():
                ...
                for shot in group_shots_list:
                    shot_trials_pnl_arr = self.calculate_shot_trials(shot, c_distance, c_buffer, c_tp, c_sl)
            ...
            if len(shot_pnl_arr) > 0:
                ...
                arr = [round(c_distance, 2), round(c_buffer, 2), round(c_tp, 2), round(c_sl, 2), round(total_pnl, 2)]
                arr_out.append(arr)
