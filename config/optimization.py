...
class StrategyOptimizationFactory(object):
    ...
    _STEP2_TOTAL_CLOSED_TRADES_VALUE_FILTER = ValueFilter("Total Closed Trades", 30, False)

    _STEP2_MAX_DRAWDOWN_PCT_VALUE_FILTER = ValueFilter("Max Drawdown, %", -30, False)

    _STEP2_EQUITY_CURVE_R_VALUE_FILTER = ValueFilter("Equity Curve R-value", 0.5, False)
    ...
    _STEP4_EQUITY_CURVE_R_VALUE_FILTER = ValueFilter("FwTest: Equity Curve R-value", 0.3, False)
    ...
    _STEP5_GROUPBY_FILTER = GroupByCombinationsFilter(
        ["Currency Pair"],
        ["FwTest: Combined Equity Curve R-value"] #"FwTest: Combined Net Profit", "FwTest: Equity Curve R-value", "FwTest: Combined Equity Curve R-value"],
    )
