...
class BacktestingStep4(object):
    ...
    _equity_curve_plotter = equity_curve()
    ...
    def parse_args(self):
        ...
        parser.add_argument('-r', '--runid', type=str, required=True, help='Name of the output file(RunId****) from Step 3')
    ...
    def get_output_equity_curve_images_path(self, base_dir, args):
        return '{}/strategyrun_results/{}/Step4_EquityCurveImages'.format(base_dir, args.runid)
    ...
    def build_equity_curve_plot_figure(self):
        equity_curve_plot = figure(plot_height=self._EQUITY_CURVE_IMAGE_HEIGHT, plot_width=self._EQUITY_CURVE_IMAGE_WIDTH, tools="", x_axis_type="datetime", toolbar_location=None,
                                   x_axis_label="Trade Number", x_axis_location="below", y_axis_label="Equity", background_fill_color="#efefef")
        equity_curve_plot.xaxis.formatter = DatetimeTickFormatter(days=["%Y-%m-%d"], months=["%Y-%m-%d"], hours=["%Y-%m-%d"], minutes=["%Y-%m-%d"])
    ...
    def draw_plot(self, row, equity_curve_data_points_str):
        ...
        text3 = "Total Closed Trades: {}, Net Profit,%: {}%, Max Drawdown,%: {}%, Max Drawdown Length: {}, Win Rate,%: {}".format(total_closed_trades, net_profit_pct,
                                                                                                                                  max_drawdown_pct, max_drawdown_length,
                                                                                                                                  win_rate_pct)
        text4 = "Winning Months,%: {}%, Profit Factor: {}, SQN: {}".format(winning_months_pct, profit_factor, sqn)
        text5 = "Equity Curve Angle={}°, Equity Curve Slope={}, Equity Curve Intercept={}, Equity Curve R-value={}, Equity Curve P-value={}, Equity Curve Stderr={}".format(
            equitycurveangle, equitycurveslope, equitycurveintercept, equitycurvervalue, equitycurvepvalue, equitycurvestderr)
    ...
    def generate_equity_curve_images(self, step2_results, step1_equity_curve_df, args):
        ...
        for row in step2_results:
            ...
            export_png(draw_column, filename=image_filename)  # show(draw_column)
