...
process_backtest() {
    ...
    python Backtesting_Step1.py -y $_strategyid -r $_runid -e $exchange -s $symbol -t $timeframe -l Fixed -z 10000 --fromyear $_fromyear --toyear $_toyear --frommonth $_frommonth --tomonth $_tomonth --fromday $_fromday --today $_today --monthlystatsprefix $monthlystatsprefix
    current_date_time="`date '+%Y-%m-%d - %H:%M:%S'`"
